...
applyManyTimes n f x
  | n > 0       = applied `seq` applyManyTimes (n - 1) f applied
  | otherwise   = x
  where applied = f x
...
ex743 = freqFilter
freqFilter :: Eq a => Int -> [a] -> [a]
freqFilter n xs = filter frequencyGreaterThen xs
  where frequencyGreaterThen x = freq x xs >= n
...
canonicalizePairs :: Ord a => [(a, a)] -> [(a, a)]
canonicalizePairs xs = map (\(x, y) -> (min x y, max x y)) $
  filter (\(x, y) -> x /= y) xs
